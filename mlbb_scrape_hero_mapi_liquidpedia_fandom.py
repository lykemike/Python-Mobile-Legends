...
for i in range(len(hero_data)):
    print("starting .... ", hero_data[i]["name"])
    url = "https://mobile-legends.fandom.com/wiki/" + hero_data[i]["name"]
    response = requests.get(url)
    soup = BeautifulSoup(response.text, "html.parser")

    hero_name = soup.find_all("span", class_="mw-page-title-main")
    ratings = soup.find_all("span", class_="bar")
    durability = "" if not ratings else ratings[0].get_text()[:-1]
    offense = "" if not ratings else ratings[1].get_text()[:-1]
    control_effect = "" if not ratings else ratings[2].get_text()[:-1]
    difficulty = "" if not ratings else ratings[3].get_text()[:-1]
    ...
    MAPI_durability = MAPI_data_response["mag"]
    MAPI_offense = MAPI_data_response["phy"]
    MAPI_control_effect = MAPI_data_response["alive"]
    MAPI_difficulty = MAPI_data_response["diff"]

    # Find hero role
    # LP_url = "https://liquipedia.net/mobilelegends/" + hero_data[i]["lp"]
    # LP_response = requests.get(LP_url)
    # LP_soup = BeautifulSoup(LP_response.text, "html.parser")

    find_role = soup.find("div", attrs={"data-source": "role"})
    role = "" if not find_role else find_role.find_all("a")  # type: ignore
    role_len = len(role)
    role_primary = ""
    role_secondary = ""

    if role_len == 1:
        role_primary = role[0].get_text()  # type: ignore
    if role_len == 2:
        role_primary = role[0].get_text()  # type: ignore
        role_secondary = role[1].get_text()  # type: ignore

    # Find hero specialty
    # LP_url = (
    #     "https://liquipedia.net/mobilelegends/" + hero_data[i]["liquidpedia_url"]
    # )
    # LP_response = requests.get(LP_url)
    # LP_soup = BeautifulSoup(LP_response.text, "html.parser")
    # LP_find_specialty = soup.find_all(string="Specialty:")
    # print(LP_find_specialty)

    find_specialty = soup.find("div", attrs={"data-source": "specialty"})
    specialty = "" if not find_specialty else find_specialty.find_all("a")  # type: ignore
    specialty_len = len(specialty)
    specialty_primary = ""
    specialty_secondary = ""

    if specialty_len == 1:
        specialty_primary = specialty[0].get_text()  # type: ignore
    if specialty_len == 2:
        specialty_primary = specialty[0].get_text()  # type: ignore
        specialty_secondary = specialty[1].get_text()  # type: ignore

    # Create a dictionary to hold the data
    data = {
        "id": hero_data[i]["id"],
        "hero_id": hero_data[i]["id"],
        "hero_name": [hero_name[0].get_text()],
        "primary_role": role_primary,
        "secondary_role": role_secondary,
        "durability": MAPI_durability,
        "offense": MAPI_offense,
        "control_effect": MAPI_control_effect,
        "difficulty": MAPI_difficulty,
        "specialty_primary": specialty_primary,
        "specialty_secondary": specialty_secondary,
    }
    ...
    try:
        df_existing = pd.read_excel("mobile_legends_heroes.xlsx")
    ...
    df_combined.to_excel("mobile_legends_heroes.xlsx", index=False)

    print("DONE! ", hero_data[i]["name"])
